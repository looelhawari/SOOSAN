...
function updateActivityFeed(activities) {
    ...
    if (!activityFeed || !activities || activities.length === 0) return;
...
function createActivityItem(activity) {
    ...
    const eventClass = activity.event === 'created' ? 'success' :
                      activity.event === 'updated' ? 'warning' : 'danger';
    const eventIcon = activity.event === 'created' ? 'plus' :
                     activity.event === 'updated' ? 'edit' : 'trash';
    ...
    itemDiv.innerHTML = `
        <div class="d-flex justify-content-between align-items-start">
            <div class="flex-grow-1">
                ...
                <p class="mb-1 text-muted">${activity.description || activity.event + ' ' + activity.auditable_type.split('\\').pop()}</p>
                <small class="text-muted">
                    <i class="fas fa-clock me-1"></i>
